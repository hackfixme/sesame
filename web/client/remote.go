...
// RemoteAuth attempts to connect to a remote Sesame server, and authenticate
...
func (c *Client) RemoteAuth(ctx context.Context, token string) (ctypes.RemoteAuthResponse, error) {
	var cresp ctypes.RemoteAuthResponse
	...
	// (CA) certificate.
	respBodyDec, err := c.RemoteJoin(ctx, base58.Encode(authToken), base58.Encode(pubKeyData))
	if err != nil {
		return cresp, err
	}
	...
	var resp stypes.RemoteJoinResponseData
...
// RemoteJoin sends a request to the remote Sesame node to authenticate the
